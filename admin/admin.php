...
function sprawdzLogowanie() {
    ...
    if (isset($_POST['zaloguj'])) {
        ...
        if ($wpisanyLogin === $login && $wpisaneHaslo === $pass) {
            ...
            header("Location: /v1.8/index.php?idp=4");
            exit();
        } else {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        echo '<tr>
                ...
                <td>
                    ...
                    <a href="index.php?idp=8&edytuj=' . $row['id'] . '" style="margin-left:8px;">Edytuj</a>
                </td>
              </tr>';
    }
    echo '</table>';
    ...
    function renderTree($matka, $tree, $level = 0) {
        if (isset($tree[$matka])) {
            foreach ($tree[$matka] as $category) {
                ...
                <a href="index.php?idp=8&edytuj_kategorie=' . $category['id'] . '">Edytuj</a>
                <form method="post" style="display:inline; margin:0; padding:0;">
                    <button type="submit" name="usun_kategorie" value="' . $category['id'] . '" 
                        style="display:inline; margin-left:8px; padding:2px 6px;">
                        Usuń
                    </button>
                </form><br>';
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        echo '<tr>
                ...
                <td style="white-space:nowrap;">
                        <a href="index.php?idp=8&usun_produkt=' . $row['id'] . '">Usuń</a>
                        <a href="index.php?idp=8&edytuj_produkt=' . $row['id'] . '">Edytuj</a>
                    </td>
              </tr>';
    }
...
echo '<a href="index.php?idp=8&lista">Lista podstron</a> |
<a href="index.php?idp=8&dodaj">Dodaj nową podstronę</a> |
<a href="index.php?idp=8&pokaz_kategorie">Pokaż kategorie</a> |
<a href="index.php?idp=8&dodaj_kategorie">Dodaj kategorię</a> |
<a href="index.php?idp=8&lista_produktow">Lista produktów</a> |
<a href="index.php?idp=8&dodaj_produkt">Dodaj produkt</a> |
<a href="index.php?idp=8&logout">Wyloguj</a><br>';
    if (isset($_GET['edytuj'])) {
        EdytujPodstrone();
